...
def test_1(p, d):
    ...
    point, has already been identified by the METOD algorithm by applying
    check_des_points(). The position of the identified local minimizer is c = 0.
    """
    lambda_1 = 1
    lambda_2 = 10
    ...
    assert(calc_minimizer_sev_quad(x, p, store_x0, matrix_test) ==
           calc_minimizer_sev_quad(iterations_of_sd[-1], p,
...
def test_2(p, d):
    ...
    Check whether a local minimizer, found by applying local descent to a
    point, has already been identified by the METOD algorithm by applying
    check_des_points(). The position of the identified local minimizer is
    c = len(discovered_minimizers)-1.
    """
    lambda_1 = 1
    lambda_2 = 10
    store_A = np.zeros((p, d, d))
...
def test_4(p, d):
    ...
    Check whether a local minimizer, found by applying local descent to a
    point, has already been identified by the METOD algorithm by applying
    check_des_points(). The position of the identified local minimizer can be
    either c = len(discovered_minimizers)-1 or c = len(discovered_minimizers)-2.
    """
    lambda_1 = 1
    lambda_2 = 10
    store_A = np.zeros((p, d, d))
