...
def inefficient_sog_grad1(point, p, sigma_sq, store_x0, matrix_test, store_c):
    """
...
def test_1():
    """Test prev_mt_alg.sog_grad() for d = 2."""
    d = 2
    p = 3
    sigma_sq = 0.05
    ...
    gradient_test = prev_mt_alg.sog_grad(x, *func_args)
...
def test_2_f():
    """Computational example for prev_mt_alg.sog_grad()."""
    d = 2
    p = 3
    sigma_sq = 0.05
    store_c = np.zeros((p, ))
...
def test_3(p, d):
    """
    Testing size output of prev_mt_alg.sog_grad() has correct form.
    """
    sigma_sq = 0.05
    lambda_1 = 1
    lambda_2 = 10
...
def test_4():
    """
    Compares results of inefficient_sog_grad1() and prev_mt_alg.sog_grad().
    ...
    x = np.random.uniform(0, 1, (d,))
    grad_val_1 = inefficient_sog_grad1(x, *func_args)
    grad_val_2 = (prev_mt_alg.sog_grad
                  (x, *func_args))
    assert(np.all(np.round(grad_val_1, 5) == np.round(grad_val_2, 5)))


def inefficient_sog_grad2(point, p, sigma_sq, store_x0, matrix_test, store_c):
    """
    Compute Sum of Gaussians gradient at a given point with given arguments.
...
def test_5():
    ...
    for i in range(p):
        ...
        c = store_c[i]
        matrix = rotation.T @ A @ rotation
        func_value = float((c) * np.exp((-1 / (2 * sigma_sq)) *
                           ((((x - x0).T) @ matrix) @ (x - x0))))
        individual_gradient = func_value * (matrix @ (x - x0))
        cumulative_gradient += individual_gradient
...
def test_6():
    ...
    gradient_test = mt_obj.sog_gradient(x, *func_args)
    el_1 = ((10 * np.exp(-9.225) * 0.99) + (12 * np.exp(-0.516) * -0.072) +
            (14 * np.exp(-0.592) * 0.268)) * sigma_sq
    el_2 = ((10 * np.exp(-9.225) * 1.755) + (12 * np.exp(-0.516) * -0.124) +
            (14 * np.exp(-0.592) * 0.324)) * sigma_sq
    assert(np.round(gradient_test[0], 5) == np.round(el_1, 5))
    assert(np.round(gradient_test[1], 5) == np.round(el_2, 5))
...
def test_8():
    """
    Compares results of inefficient_sog_grad2() and mt_obj.sog_gradient().
    Results should be the same.
    """
    p = 10
    d = 20
    ...
    x = np.random.uniform(0, 1, (d,))
    grad_val_1 = inefficient_sog_grad2(x, *func_args)
