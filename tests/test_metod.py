...
def test_1():
    ...
    if isinstance(num_points, int) is False:
        ans_1 = False
    assert(ans_1 is False)
    num_points = 100
    ans_2 = True
    if isinstance(num_points, int) is False:
        ans_2 = False
    assert(ans_2 is True)
    proj = 0.999
    ans_3 = True
    if isinstance(proj, bool) is False:
        ans_3 = False
    assert(ans_3 is False)
    proj = True
    ans_4 = True
    if isinstance(proj, bool) is False:
        ans_4 = False
    assert(ans_4 is True)


@settings(max_examples=10, deadline=None)
@given(st.integers(2, 20), st.integers(0, 3), st.integers(2, 100))
def test_2(p, m, d):
    """ Test warm up, m, is being applied correctly in metod when computing
     distances """
    np.random.seed(p)
    ...
    iterations_of_sd, its = (mtv3.apply_sd_until_stopping_criteria
                             (x, d, projection, tolerance, option, met,
                              initial_guess, func_args, f, g, bound_1=1,
                              bound_2=0))
