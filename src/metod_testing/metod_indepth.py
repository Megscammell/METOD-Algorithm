...
def metod_indepth(f, g, func_args, d, num_points=1000, beta=0.01,
                  ...
                  initial_guess=0.05, set_x=np.random.uniform,
                  bounds_set_x=(0, 1)):
    """Returns total number of minima discovered by algorithm, position of
    local minima, function values of local minima, number of unnecessary
    descents, number of iterations of descended points, descended points,
    partner points of descended points and all starting points
    ...
    const --  a constant for the minimum euclidean distance to be larger than
     to classify a point as a new local minima
    m -- warm up period
    option - choose from 'minimize' or 'minimize_scalar'. Default is
     'minimize'.
    met -- choose appropiate method for option. Default is 'Nelder-Mead'.
    initial_guess -- is passed to the scipy.optimize.minimize function. This
     is recommended to be small (0.05). Method chosen is Nelder-Mead.
    set_x -- Generates points for the METOD algorithm. Can be either inbuilt
     numpy.random distribution that generates points, list or numpy array of
     size num_points
    bounds_set_x - Bounds for numpy.random distribution
    """
    if isinstance(d, int) is False:
        raise ValueError('d must be an integer.')
    if isinstance(num_points, int) is False:
        raise ValueError('num_points must be an integer.')
    if isinstance(beta, float) is False:
        raise ValueError('beta must be a float.')
    if isinstance(tolerance, float) is False:
        raise ValueError('tolerance must be a float.')
    if isinstance(projection, bool) is False:
        raise ValueError('projection must be boolean.')
    if isinstance(const, float) is False:
        raise ValueError('const must be a float.')
    if isinstance(m, int) is False:
        raise ValueError('m must be an integer.')
    if isinstance(option, str) is False:
        raise ValueError('option must be a string.')
    if isinstance(met, str) is False:
        raise ValueError('met must be a string.')
    if isinstance(initial_guess, float) is False:
    ...
    for remaining_points in tqdm.tqdm(range(num_points - 1)):
        ...
        if possible_regions == []:
            ...
            if (its + m) <= m:
                raise ValueError('m is equal to or larger than the total'
                                 'number of steepest descent iterations to '
                                 'find a minimizer. Please change m or change '
                                 'tolerance.')
            iterations_of_sd = np.vstack([warm_up_sd,
                                          iterations_of_sd_part[1:, ]])
            des_x_points.append(iterations_of_sd)
            starting_points[remaining_points + 1, :] = (iterations_of_sd[0, :].
                                                        reshape(1, d))
            ...
            starting_points[remaining_points + 1, :] = (warm_up_sd[0].reshape
                                                        (1, d))
    ...
    return (unique_minima, unique_number_of_minima, func_vals_of_minimas,
            (len(des_x_points) - unique_number_of_minima), store_its,
            des_x_points, des_z_points, starting_points)
