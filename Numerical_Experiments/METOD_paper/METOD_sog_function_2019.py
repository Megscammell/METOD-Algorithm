...
def check_classification_points_metod(classification_points,
    ...
    for j in range(len(classification_points)):
        class_store_x0[j] = (mt_obj.calc_minimizer_sog
                             (unique_minimizers_metod[int(classification_points[j])],
                              *func_args))
    return class_store_x0


def metod_numerical_exp_sog_new(f, g, func_args, d,
...
def metod_numerical_exp_sog_original(f, g, func_args, d, num_p, beta,
                                     tolerance, projection, const, m,
                                     option, met, initial_guess, set_x,
                                     bounds_set_x, relax_sd_it, sd_its,
                                     check_func):
    """
    Apply the METOD algorithm with specified parameters. If sd_its =
    True, multistart will also be applied with the same starting points
...
@dask.delayed
def all_functions_metod(f, g, p, lambda_1, lambda_2, sigma_sq, d,
                        num_p, beta, tolerance, projection,
                        const, m, option, met, initial_guess,
                        set_x, bounds_set_x, relax_sd_it, sd_its,
        ...
        if sd_its == True:
            if type_func == 'old':
                (number_minimizers_per_func_multistart[func],
                 ...
                 store_total_checks[func]) = (metod_numerical_exp_sog_original
                                              (f, g, func_args, d,
                ...
                (number_minimizers_per_func_multistart[func],
                 ...
                 starting_points,
                 store_missed_minimizers[func],
                 store_total_checks[func]) = (metod_numerical_exp_sog_new
                                              (f, g, func_args, d,
                                               num_p, beta, tolerance,
                                               projection, const, m, option,
                                               met, initial_guess, set_x,
...
if __name__ == "__main__":
    ...
    num_p : 1000.
    beta : set beta to be either 0.005, 0.01, 0.05, 0.1 or 0.2.
    m : set warm up period to be either 2, 3, or 4.
    set_x : 'random'.
    sd_its : True.
    p : set p = 20 for results in [1]. Otherwise, p = 10 for results in thesis.
    ...
    initial_guess : set initial_guess = 0.005.
    ...
    task = all_functions_metod(f, g, p, lambda_1, lambda_2, sigma_sq, d,
                               num_p, beta, tolerance, projection,
                               const, m, option, met, initial_guess,
                               set_x, bounds_set_x, relax_sd_it, sd_its,
                               check_func, num_func, random_seed, type_func)
