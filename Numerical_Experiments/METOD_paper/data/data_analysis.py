    ...
    for beta in beta_list:
        for m in m_list:
            ...
            if func_name == 'quad':
                ...
                data_table[index, 3] = np.where(np.array(df["number_extra_descents_per_func_metod"]) + np.array(df['number_minimizers_per_func_metod']) == p)[0].shape[0] 
                data_table[index, 4] = np.where(np.array(df["number_extra_descents_per_func_metod"]) + np.array(df['number_minimizers_per_func_metod']) == p- 1)[0].shape[0] 
                data_table[index, 5] = np.where(np.array(df["number_extra_descents_per_func_metod"]) + np.array(df['number_minimizers_per_func_metod']) <= p- 2)[0].shape[0] 
                ...
                data_table[index, 3] = np.min(np.array(df["number_extra_descents_per_func_metod"]) + np.array(df['number_minimizers_per_func_metod']))
                data_table[index, 4] = np.max(np.array(df["number_extra_descents_per_func_metod"]) + np.array(df['number_minimizers_per_func_metod']))
                data_table[index, 5] = np.mean(np.array(df["number_extra_descents_per_func_metod"]) + np.array(df['number_minimizers_per_func_metod'])) 
...
def write_to_latex(data_table_pd, func_name, d, p, set_x, sigma_sq, num_p, option, initial_guess):
    if func_name == 'quad':
        ...
        with open('%s_data_table_d=%s_p=%s_%s_%s_%s_%s.tex' 
                  % (func_name, d, p, set_x, num_p, option[0], initial_guess), 'w') as tf:
            tf.write(data_table_pd.to_latex())
            
    elif func_name == 'sog':
        data_table_pd.to_csv('%s_data_table_d=%s_p=%s_%s_%s_%s_%s_%s.csv'
                             % (func_name, d, p, set_x, sigma_sq, num_p, option[0], initial_guess))
        with open('%s_data_table_d=%s_p=%s_%s_%s_%s_%s_%s.tex' 
                  % (func_name, d, p, set_x, sigma_sq, num_p, option[0], initial_guess), 'w') as tf:
            tf.write(data_table_pd.to_latex())
