...
if __name__ == "__main__":
    ...
    num_p : 1000.
    beta : set beta to be either 0.001, 0.005 or 0.01.
    m : set warm up period to be either 1 or 2.
    set_x : 'random'.
    ...
    np.savetxt('stybl_tang_grad_norm_beta_%s_m=%s_d=%s'
               '_%s_%s_%s_%s.csv' %
               (beta, m, d, set_x, num_p, option[0], initial_guess),
               store_grad_norms,
               delimiter=',')
    np.savetxt('stybl_tang_grad_evals_metod_beta_%s_m=%s_d=%s'
               '_%s_%s_%s_%s.csv' %
               (beta, m, d, set_x, num_p, option[0], initial_guess),
    ...
    if sd_its == True:
        ...
        table.to_csv(table.to_csv
                     ('stybl_tang_sd_metod_beta_%s_m=%s_d=%s'
                      '_%s_%s_%s_%s.csv' %
                      (beta, m, d, set_x, num_p, option[0], initial_guess)))

        np.savetxt('stybl_tang_grad_evals_mult_beta_%s_m=%s_d=%s'
                   '_%s_%s_%s_%s.csv' %
                   (beta, m, d, set_x, num_p, option[0], initial_guess),
                   store_grad_evals_mult,
                   delimiter=',')
        np.savetxt('stybl_tang_sd_start_p_beta_%s_m=%s_d=%s'
                   '_%s_%s_%s_%s.csv' %
                   (beta, m, d, set_x, num_p, option[0], initial_guess),
        ...
        table = pd.DataFrame({
                            ...
                            "min_func_val_metod": func_val_metod})
        table.to_csv(table.to_csv
                     ('stybl_tang_metod_beta_%s_m=%s_d=%s'
                      '_%s_%s_%s_%s.csv' %
                      (beta, m, d, set_x, num_p, option[0], initial_guess)))
        np.savetxt('stybl_tang_start_p_beta_%s_m=%s_d=%s'
                   '_%s_%s_%s_%s.csv' %
                   (beta, m, d, set_x, num_p, option[0], initial_guess),
                   store_starting_points,
                   delimiter=',')
