...
def compute_boxplots(times_50, times_100, met_list, p):
    ...
    plt.tight_layout()
    plt.savefig('time_quad_d_p_%s.pdf' % (p))
...
if __name__ == "__main__":
    ...
    index = 0
    for met in met_list:
        df2_50 = pd.read_csv('quad_sd_minimize_met_%s_beta_%s'
                             '_m=%s_d=%s_p=%s.csv' % (met, beta, m, 50, p))
        df1_50 = pd.read_csv('quad_testing_minimize_met_%s_beta_%s'
                             '_m=%s_d=%s_p=%s.csv' % (met, beta, m, 50, p))
        test_mets_50[index, :] = np.array(df2_50['number_minimas_'
                                                 'per_func_multistart'])
        df2_100 = pd.read_csv('quad_sd_minimize_met_%s_beta_%s'
                              '_m=%s_d=%s_p=%s.csv' % (met, beta, m, 100, p))
        df1_100 = pd.read_csv('quad_testing_minimize_met_%s_beta_%s'
                              '_m=%s_d=%s_p=%s.csv' % (met, beta, m, 100, p))
