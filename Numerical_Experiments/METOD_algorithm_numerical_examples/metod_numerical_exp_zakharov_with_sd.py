...
def check_zakharov_min(unique_minimizers, func_vals_of_minimizers, d):
    assert(len(unique_minimizers) == 1)
    assert(len(func_vals_of_minimizers) == 1)
    assert(np.all(np.round(unique_minimizers[0], 3) == np.zeros((d))))
    assert(np.round(func_vals_of_minimizers[0], 1) == 0)
...
def metod_numerical_exp(f, g, func_args, d,
    ...
    check_zakharov_min(unique_minimizers_metod, func_vals_of_minimizers_metod, d)
    ...
    if sd_its == True:
        ...
        check_zakharov_min(unique_minimizers_mult, store_func_vals_mult, d)
...
if __name__ == "__main__":
    ...
    np.savetxt('zak_grad_norm_beta_%s_m=%s_d=%s'
                '_%s_%s.csv' %
    ...
    if sd_its == True:
        ...
        table.to_csv(table.to_csv
                    ('zak_sd_metod_beta_%s_m=%s_d=%s'
                    '_%s_%s.csv' %
                    (beta, m, d, set_x, num_p)))
        
        np.savetxt('zak_no_its_mult_beta_%s_m=%s_d=%s'
                    '_%s_%s.csv' %
