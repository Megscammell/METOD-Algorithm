...
* :math:`X = {x_1,x_2,\ldots,x_N}`: a sequence of points in :math:`\mathfrak{X}`;
...
* :math:`l`: index for the local minimizers :math:`l=1,\ldots,L`, where :math:`L` is the total number of local minimizers found so far;
...
The first main condition of the METOD Algorithm tests the following condition for each :math:`l=1,\ldots,L` and all :math:`i=M-1,\ldots,K_l`
...
For a given :math:`l`, if condition :eq:`sd2` holds for all :math:`i=M-1,\ldots,
...
any :math:`l=1,\ldots,L`, then we apply anti-gradient descent iterations 
...
:math:`i=1,\ldots,L` and :math:`j=i + 1,\ldots,L`
...
removed from the set of indices :math:`l=1,\ldots,L`.
...
2) **Terminate anti-gradient descent iterations for** \ :math:`n`\ **-th   point**.
    ...
    For :math:`n=2` to :math:`N`
       ...
       For :math:`l=1` to :math:`L`
          ...
          If condition :eq:`sd2` is satisfied for every :math:`i=M-1,\ldots,K_l`
    ...
    For :math:`i=1` to :math:`L`
       ...
       For :math:`j=i+1` to :math:`L`
          ...
          If condition :eq:`sd3` is not satisfied for :math:`{x}_{i}^{(K_i)}` and :math:`{x}_{j}^{(K_j)}`
             ...
             Remove index :math:`j` from the set of indices :math:`l=1,\ldots,L`.
